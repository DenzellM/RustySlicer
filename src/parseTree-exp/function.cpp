...
funcExp::funcExp(std::string n, funcParamList *p, std::string  ret, Exp b) : name(n), body(b){
    ...
    //std::cout << "init funcExp: "<< n << ", " << params.size() << " vars, ret: " << ret << std::endl;

}

double funcExp::interp(std::shared_ptr<Table> table){
    //std::cout << "interp funcExp, added: "<< name << " to function table"<< std::endl;
    table->update(name, this);
    return std::nan("1");
}

double funcExp::interp(std::shared_ptr<Table> table, std::vector<Exp> exps){
...
Type funcExp::typeCheck(std::shared_ptr<Table> table){
...
void funcExp::checkParam(std::shared_ptr<Table> table, std::vector<Exp> exps){
